...
void moneyTransfer();
void printCentered(const string& text, int color = 10);
...
// Main function with centered output
int main() {
    int choice;
    while (true) {
        try {
            ...
            if (isLoggedIn) {
                printCentered("========== Banking Management System ==========", 14);
                Sleep(300);
                
                // Center user info
                string userInfo = "User: " + loggedInUserFullname + " | Email: " + loggedInUserEmail + " | Phone: " + loggedInUserPhoneNumber;
                printCentered(userInfo, 10);
                Sleep(300);

                printCentered("1. Deposit Money");
                Sleep(300);
                printCentered("2. Withdraw Money");
                Sleep(300);
                printCentered("3. Check Account Balance");
                Sleep(300);
                printCentered("4. Account Statement");
                Sleep(300);
                printCentered("5. Money Transfer");
                Sleep(300);
                printCentered("6. Logout");
                Sleep(300);
                printCentered("Enter your choice [1-6]: ");
                Sleep(300);

                // Move cursor to center for input
            ...
            printCentered("Error: " + string(e.what()), 12); // Red color for errors
            cin.clear();
...
void Login()
{
    try
    {
        ...
        while (getline(userCheck, line)) {
   			 ...
   			 double fileBalance;
   		 stringstream ss(line);
             getline(ss, fileFullname, ',');  // Use comma as delimiter
    	ss >> fileEmail >> filePhone >> filePassword >> filePin >> fileBalance;
